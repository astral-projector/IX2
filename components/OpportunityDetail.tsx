  ...
  return (
    <div className="border border-navy-100 rounded-sm overflow-hidden">
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 bg-navy-50 hover:bg-navy-100 transition-colors text-left"
      >
        {open ? (
          <FolderOpen size={16} className="text-navy-400 shrink-0" />
        ) : (
          <Folder size={16} className="text-navy-400 shrink-0" />
        )}
        <span className="text-sm font-medium text-navy-800">{name}</span>
        <span className="ml-auto text-xs text-navy-400">{files.length} files</span>
      </button>
      {open && (
          ...
          {files.map((file) => (
            <div key={file.name} className="relative">
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-navy-50 transition-colors text-left opacity-60 cursor-pointer"
              >
                <FileText size={14} className="text-navy-300 shrink-0" />
                <span className="text-sm text-navy-600 flex-1">{file.name}</span>
                <span className="text-xs text-navy-400">{file.date}</span>
                <span className="text-xs text-navy-400 w-14 text-right">{file.size}</span>
                <Lock size={12} className="text-navy-300 shrink-0" />
              </button>
              {tooltip === file.name && (
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview">
            <div className="grid lg:grid-cols-3 gap-10">
              <div className="lg:col-span-2 space-y-8">
                <div>
                  <h2 className="text-xs uppercase tracking-widest text-navy-400 font-medium mb-3">
                    Capital Requirement
                  </h2>
              ...
              <div>
                ...
                <ul className="space-y-2.5">
                  {opp.investmentHighlights.map((h, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-sm text-navy-700 leading-snug">
                      <span
          ...
          <TabsContent value="impact-room">
            <div className="max-w-3xl space-y-10">
              {/* Positive Pursuit */}
              <div>
                <h2 className="text-xs uppercase tracking-widest text-navy-400 font-medium mb-4">
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  {(["input", "output", "outcome", "impact"] as const).map((key, i) => (
                    <div
                      key={key}
                      className="rounded-sm border border-navy-100 p-4 bg-navy-50"
                    >
              ...
              <div>
                ...
                <div className="divide-y divide-navy-100 border border-navy-100 rounded-sm overflow-hidden">
                  {opp.outcomeKpis.map((kpi, i) => (
                    <div key={i} className="flex items-start gap-4 px-4 py-4 bg-white">
                      <div className="flex-1">
                        <div className="text-sm font-medium text-navy-800 mb-0.5">
                          {kpi.label}
                        </div>
                        <div className="text-xs text-navy-500">{kpi.unit}</div>
                      </div>
                      <div className="text-right shrink-0">
                        {kpi.value ? (
                          <div className="text-lg font-display font-light text-navy-900">
                            {kpi.value}
                          </div>
                        ) : (
                          <div className="text-xs text-navy-400 italic max-w-[160px] text-right">
                            {opp.status === "coming-soon"
                              ? "Final KPIs to be confirmed at listing"
                              : "—"}
                          </div>
                        )}
                        <div className="text-xs text-navy-400 capitalize">
              ...
              {/* Assurance & reporting */}
              <div className="space-y-3 text-sm text-navy-500 border-t border-navy-100 pt-6">
                <div className="flex items-start gap-2">
                  <span className="text-navy-400 shrink-0">Reporting cadence:</span>
                  <span className="text-navy-700 capitalize font-medium">
                    Reported {opp.reportingCadence.replace("-", " ")}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-navy-400 shrink-0">Independent assurance:</span>
                  <span className="text-navy-700 font-medium">
                    {opp.assuranceProvider ?? "To be confirmed at listing"}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-navy-400 shrink-0">Break-Even attestation:</span>
        ...
        {/* CTA */}
        <div className="mt-12 pt-8 border-t border-navy-100 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <button
          ...
          <p className="text-xs text-navy-400 leading-relaxed max-w-sm">
